<%@ WebHandler Language="C#" Class="code128" %>
...
public class code128 : IHttpHandler {

    private DataTable m_Code128;
    ...
    private string code;
    private StringBuilder strSvg;
    public bool IsReusable { get { return false;  }}
        
    public void ProcessRequest (HttpContext context) {
        
        strSvg = new StringBuilder("<?xml version=\"1.0\" encoding=\"utf-8\"?>\n");
    ...
    private void GetCodeImage(Encode p_Code)
    {
        string _ViewText = code;
        string _Text = "";
        List<int> _TextNumb = new List<int>();
        int _Examine = 0; //首位
        switch (p_Code)
        {
            case Encode.Code128C:
                _Examine = 105;
                if (!((code.Length & 1) == 0)) throw new Exception("128C長度必須是偶數");
                while (code.Length != 0)
                {
                    int _Temp = 0;
                    try
                    ...
                    _Text += GetValue(p_Code, code.Substring(0, 2), ref _Temp);
                    _TextNumb.Add(_Temp);
            ...
            case Encode.EAN128:
                _Examine = 105;
                if (!((code.Length & 1) == 0)) throw new Exception("EAN128長度必須是偶數");
                _TextNumb.Add(102);
                _Text += "411131";
                while (code.Length != 0)
                {
                    ...
                    _Text += GetValue(Encode.Code128C, code.Substring(0, 2), ref _Temp);
                    _TextNumb.Add(_Temp);
                    code = code.Remove(0, 2);
                }
                break;
            default:
                ...
                {
                    int _Temp = 0;
                    string _ValueCode = GetValue(p_Code, code.Substring(0, 1), ref _Temp);
                    if (_ValueCode.Length == 0) throw new Exception("不正確字元集!" + code.Substring(0, 1).ToString());
                    _Text += _ValueCode;
                    _TextNumb.Add(_Temp);
    ...
    /// <returns>圖形</returns>
    private void GetImage(string p_Text)
    {
        ...
        strSvg.AppendFormat("width=\"{0}\" height=\"{1}\" >", _Width, (int)m_Height);
        strSvg.AppendFormat("\t<rect x=\"{0}\" y=\"{1}\" stroke=\"black\" fill=\"none\" width=\"{2}\" height=\"{3}\"/>\n", 0, 0, _Width, (int)m_Height);
        ...
        for (int i = 0; i != _Value.Length; i++)
        {
            ...
            {
                //白色条
                strSvg.AppendFormat("\t<rect x=\"{0}\" y=\"{1}\" fill=\"#ffffff\" width=\"{2}\" height=\"{3}\"/>\n", _LenEx, 0,   _ValueNumb, (int)m_Height);
            }
            else
            {
                //黑色条
        ...
        strSvg.AppendFormat("\t<text style=\"fill:#000000;font-size:24px\" x=\"{2}\" y=\"{1}\">*{0}*</text>", code, m_Height + 24, _Width / 2 - code.Length * 12);
    }
    
    private void initDataTable()
    {
        m_Code128 = new DataTable();
